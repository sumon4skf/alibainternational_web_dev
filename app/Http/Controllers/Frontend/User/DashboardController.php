...
class DashboardController extends Controller
{
    ...
    {
        $order = Order::with(['orderItems' => function ($items) {
            $items->with('itemVariations');
        }])
            ->where('id', $order_id)
    ...
    {
        $invoice = Invoice::with('invoiceItems')->where('invoice_no', $invoice_id)->where('user_id', auth()->id())->firstOrFail();
